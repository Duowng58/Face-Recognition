"""Face tracker – no PySide6 dependency.

Replaces PySide6 Signal with a plain callback:
    tracker.on_disappeared = my_handler   # callable(track_id: int)
"""
...
# ──────────────────────────────────────────────
# Kalman filter for center-point tracking
# ──────────────────────────────────────────────
...
def greedy_match(cost_matrix, threshold):
    ...
    matched, used_det, used_trk = [], set(), set()
...
class Tracker:
    def __init__(
        ...
        # Callback: called with (track_id: int) when a track disappears
    ...
    def update(self, objects_rect, classId="face"):
        ...
        disappeared_ids = [
            ...
            if self.id_to_class.get(tid) == classId and tid in self.kalman and tid in self.id_to_bbox
        ]
        reidentified_det: set = set()
        if disappeared_ids:
            ...
            for di in unmatched_dets:
                ...
                best_tid = None; best_dist = float("inf")
                for tid in disappeared_ids:
                    lx1, ly1, lx2, ly2 = self.id_to_bbox[tid]
                    last_cx = (lx1 + lx2) / 2; last_cy = (ly1 + ly2) / 2
                ...
                if best_tid is not None:
                    ...
                    self.disappeared.pop(best_tid, None)
                    objects_now.append([x1, y1, x2, y2, best_tid, classId])
                    matched_det.add(di); reidentified_det.add(di)
        ...
        for di, rect in enumerate(objects_rect):
            ...
            x1, y1, x2, y2 = rect
            cx = (x1 + x2) / 2; cy = (y1 + y2) / 2
            new_id = self.id_count; self.id_count += 1
        ...
        # Increment disappearance counters
        ...
        to_remove = [tid for tid, cnt in self.disappeared.items() if cnt > self.max_disappeared]
        ...
        self.objects_bbs_ids = [obj for obj in self.objects_bbs_ids if obj[5] != classId]
