...
class MongoDBClient:
	...
	Example:
		...
		client.connect()
		items = client.find("attendance", {"class_code": "Web-3"})
	"""
...
def to_local(dt_value: Any) -> datetime:
	...
	if dt_value.tzinfo is None:
		# assume stored value is UTC when naive
...
class StudentRepository:
	...
	def insert(self, student: Student) -> ObjectId:
		student_id = self._client.collection(self._collection).insert_one(student.to_document()).inserted_id
		return student_id
...
class AttendanceRepository:
	...
	def insert(self, attendance: Attendance) -> ObjectId:
		attendance_id = self._client.collection(self._collection).insert_one(attendance.to_document()).inserted_id
